...
def fuse_conv2d_bias_add_relu_max_pool2d():
    ...
    dequantized_o = dfp.is_op("qnn.dequantize")(o,dfp.wildcard(),dfp.wildcard())
    max_pool2d_op = dfp.is_op("nn.max_pool2d")(dequantized_o)
...
def fuse_flatten():
    ...
    i = dfp.wildcard()  # Input
    dequantized_i = dfp.is_op("qnn.dequantize")(i,dfp.wildcard(),dfp.wildcard())
    batch_flatten_op = dfp.is_op("nn.batch_flatten")(dequantized_i)
    quantize_op = dfp.is_op("qnn.quantize")(batch_flatten_op,dfp.wildcard(),dfp.wildcard())
    cast_op = dfp.is_op("cast")(quantize_op)  # Assuming requantize is a cast operation
    return cast_op
    #! implement<<
