...
class CodegenC(CodegenCBase):
    ...
    def get_conv_info(self,call):
        ...
        while len(op_list) > 0:
            ...
            if op.op.name == "nn.conv2d":
                ...
                conv2d_info["m"] = conv2d_info["M"] # set the same
            #! implement<<
            for node in op.args:
    ...
    def visit_call(self, call):
        ...
        #! implement>>
        for arg,notes in zip(call.args,tvm_auto_args_NOTES[func_name]):
            if isinstance(arg, Constant):
                parameters[notes] = (arg, True) # if it is constant, do not visit first
            else:
                res = self.visit_expr(arg)
                parameters[notes] = (res[0], False)
        #! implement<<
        ...
        #buf_stream = f"{dtype}* {out} = ({dtype}*)malloc({out_size * 4});"
        #! implement<<
